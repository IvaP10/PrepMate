...
test.describe("authenticated application surfaces", () => {
  ...
  test("all primary and account destinations render real backend state without runtime failures", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: "Interview Performance", exact: true })).toBeVisible()
    await expect(page.getByRole("heading", { name: "Communication", exact: true })).toBeVisible()
    await expect(page.getByRole("heading", { name: "Technical Performance", exact: true })).toBeHidden()
    await performanceMode.getByRole("button", { name: "Technical Round", exact: true }).click()
    await expect(page.getByRole("heading", { name: "Technical Performance", exact: true })).toBeVisible()
    await expect(page.getByRole("heading", { name: "Communication", exact: true })).toBeHidden()
