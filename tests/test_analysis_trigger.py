...
def test_analysis_trigger_is_monotonic_for_ready_and_running_reports():
    ...
    assert _analysis_trigger_decision("report_ready", True) == "ready"
...
def test_performance_reconciliation_requeues_missing_canonical_analyses():
    ...
    execute = AsyncMock(return_value=[("interview-1",), ("interview-2",)])
    with patch.object(analysis, "async_execute", execute), patch.object(
        analysis,
        "enqueue_analysis",
        enqueue,
    ):
